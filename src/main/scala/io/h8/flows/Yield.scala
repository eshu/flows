...
object Yield {
  final case class Some[-I, +O, +E](out: O, onDone: OnDone[I, O, E]) extends Yield[I, O, E] {
    private[flows] def ~>[_I >: O, _O, _E >: E](flow: Flow[_I, _O, _E]): Yield[I, _O, _E] = flow(out) match {
      case Some(_out, _onDone) => Some(_out, onDone <~ _onDone)
      case None(_onDone)       => None(onDone <~ _onDone)
    }
  }
